...
object Par {
  ...
  def map2[A,B,C](a: Par[A], b: Par[B])(f: (A,B) => C): Par[C] =
    (es: ExecutorService) => {
      ...
      // waits for the returned futures. This means that even if you have
      // passed in "forked" arguments, using this map2 on them will make them
      // wait. It simply passes the `ExecutorService` on to both `Par` values,
      // waits for the results of the Futures `af` and `bf`, applies `f` to
      // them, and wraps them in a `UnitFuture`. In order to respect timeouts,
      // we'd need a new `Future` implementation that records the amount of
      // time spent evaluating `af`, then subtracts that time from the
      // available time allocated for evaluating `bf`.
      UnitFuture(f(af.get, bf.get))
    }

  // This is the simplest and most natural implementation of `fork`, but there
  // are some problems with it--for one, the outer `Callable` will block
  // waiting for the "inner" task to complete. Since this blocking occupies a
  // thread in our thread pool, or whatever resource backs the
  // `ExecutorService`, this implies that we're losing out on some potential
  // parallelism. Essentially, we're using two threads when one should suffice.
  // This is a symptom of a more serious problem with the implementation, and
